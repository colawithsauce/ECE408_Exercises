...
__global__ void matrix_multi_kernel1(double *A_d, double *B_d, double *C_d, int M, int N, int S)
{
    unsigned int col = threadIdx.x + blockDim.x * blockIdx.x;

    // write a kernel that has each thread produce one output matrix row. Fill in the execution configuration
    // parameters for the disign.
    if (col < N)
    {
...
__global__ void matrix_multi_kernel2(double *A_d, double *B_d, double *C_d, int M, int N, int S)
{
    unsigned int row = threadIdx.y + blockDim.y * blockIdx.y;

    // write a kernel that has each thread produce one output matrix column, Fill in the execution configuration
    // parameters for the disign.
    if (row < M)
    {
...
cudaError_t matrix_multi(const double *A_h, const double *B_h, double *C_h, int M, int N, int S)
{
    ...
    start = clock();
    matrix_multi_kernel1 KERNEL_ARGS2(dimGrid1, dimBlock1)(A_d, B_d, C_d, M, N, S);
    elapsed = 1000 * (double)(clock() - start) / CLOCKS_PER_SEC; // in milliseconds
    printf("row matrix_multi1: %lf ms\n", elapsed);

    start = clock();
    matrix_multi_kernel2 KERNEL_ARGS2(dimGrid2, dimBlock2)(A_d, B_d, C_d, M, N, S);
    elapsed = 1000 * (double)(clock() - start) / CLOCKS_PER_SEC; // in milliseconds
