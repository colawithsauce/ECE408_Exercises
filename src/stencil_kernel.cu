...
__global__ void stencil_kernel(float *A, float *B, unsigned int N, const int *c)
{
    ...
    {
        B[i * N * N + j * N + k] = c[0] * A[i * N * N + j * N + k] + c[1] * A[(i + 1) * N * N + j * N + k] +
                                   c[2] * A[(i - 1) * N * N + j * N + k] + c[3] * A[i * N * N + (j + 1) * N + k] +
                                   c[4] * A[i * N * N + (j - 1) * N + k] + c[5] * A[i * N * N + j * N + k + 1] +
                                   c[6] * A[i * N * N + j * N + k - 1];
    }
}

__global__ void stencil_with_tiling(float *in, float *out, unsigned int N, const int *c)
{
    ...
    if (i >= 1 && i < N - 1 && j >= 1 && j < N - 1 && k >= 1 && k < N - 1)
    {
        ...
        {
            out[i * N * N + j * N + k] = c[0] * in_s[threadIdx.z][threadIdx.y][threadIdx.x] +
...
__global__ void stencil_with_coarsening(float *in, float *out, unsigned int N, const int *c)
{
    ...
    if (j >= 0 && k >= 0 && j < N && k < N && iStart - 1 >= 0 && iStart - 1 < N)
    {
        prev[threadIdx.y][threadIdx.x] = in[(iStart - 1) * N * N + j * N + k]; // NOTE: use tx and ty here
    }
    ...
    for (int i = iStart; i != iStart + OUT_TILE_DIM; ++i)
    {
        ...
        if (i >= 1 && i < N - 1 && j >= 1 && j < N - 1 && k >= 1 && k < N - 1)
        {
            ...
            {
                out[i * N * N + j * N + k] =
                    c[0] * curr[threadIdx.y][threadIdx.x] + c[1] * next[threadIdx.y][threadIdx.x] +
                    c[2] * prev[threadIdx.y][threadIdx.x] + c[3] * curr[threadIdx.y + 1][threadIdx.x] +
                    c[4] * curr[threadIdx.y - 1][threadIdx.x] + c[5] * curr[threadIdx.y][threadIdx.x + 1] +
...
__global__ void stencil_with_amend_conarsening(float *in, float *out, unsigned int N, const int *c)
{
    int iStart = blockIdx.z * OUT_TILE_DIM;
    int j = blockIdx.y * OUT_TILE_DIM + threadIdx.y - 1;
    int k = blockIdx.x * OUT_TILE_DIM + threadIdx.x - 1;
    ...
    for (int i = iStart; i != iStart + OUT_TILE_DIM; ++i)
    {
        ...
        if (i >= 1 && i < N - 1 && j >= 1 && j < N - 1 && k >= 1 && k < N - 1)
        {
            if (tz >= 1 && tz < IN_TILE_DIM - 1 && ty >= 1 && ty < IN_TILE_DIM - 1 && tx >= 1 && tx < IN_TILE_DIM - 1)
            {
                out[i * N * N + j * N + k] = c[0] * curr[ty][tx] + c[1] * prev + c[2] * next + c[3] * curr[ty + 1][tx] +
                                             c[4] * curr[ty - 1][tx] + c[5] * curr[ty][tx + 1] +
                                             c[6] * curr[ty][tx - 1];
            }
        }
