...
__global__ void stencil_kernel(float* A, float* B, unsigned int N,
                               const int* c) {
...
__global__ void stencil_with_tiling(float* in, float* out, unsigned int N,
                                    const int* c) {
...
__global__ void stencil_with_coarsening(float* in, float* out, unsigned int N,
                                        const int* c) {
  ...
  for (int i = iStart; i != iStart + OUT_TILE_DIM; ++i) {
    ...
    if (i >= 1 && i < N - 1 && j >= 1 && j < N - 1 && k >= 1 && k < N - 1) {
      if (threadIdx.x >= 1 && threadIdx.x < IN_TILE_DIM - 1 &&
        ...
        out[i * N * N + j * N + k] = c[0] * curr[threadIdx.y][threadIdx.x] +
...
__global__ void stencil_with_amend_conarsening(float* in, float* out,
                                               unsigned int N, const int* c) {
  int iStart = blockIdx.z * OUT_TILE_DIM;
  int j = blockIdx.y * OUT_TILE_DIM + threadIdx.y - 1;
  int k = blockIdx.x * OUT_TILE_DIM + threadIdx.x - 1;
  ...
  for (int i = iStart; i != iStart + OUT_TILE_DIM; ++i) {
    ...
    if (i >= 1 && i < N - 1 && j >= 1 && j < N - 1 && k >= 1 && k < N - 1) {
      if (tz >= 1 && tz < IN_TILE_DIM - 1 && ty >= 1 && ty < IN_TILE_DIM - 1 &&
