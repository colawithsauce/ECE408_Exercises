...
//   C_d: M * N matrix
__global__ void
matrix_multi_tile_kernel(const double* A_d,
                         const double* B_d,
                         double* C_d,
                         int M,
                         int N,
                         ...
                         int nBlkWidth)
{
    ...
    double Cvalue = 0;
    for (int ph = 0; ph < ceil(max(M / (float)nBlkWidth, N / (float)nBlkWidth));
         ph++) {
                  ...
                  int N)
{
    ...
    matrix_multi_tile_kernel<<<dimGrid, dimBlock, 1024 * 2 * sizeof(double)>>>(
      A_d, B_d, C_d, M, N, S, 1024, 32);
