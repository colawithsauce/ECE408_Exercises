              ...
              const int MASK_WIDTH)
{
    ...
    for (int j = 0; j != MASK_WIDTH; ++j) {
        for (int i = 0; i != MASK_WIDTH; ++i) {
            if (row_i + j >= 0 && col_i + i >= 0 && row_i + j < size &&
                col_i + i < size) {
                Pvalue +=
                  M[j * MASK_WIDTH + i] * N[(row_i + j) * size + col_i + i];
            }
        }
    }
...
// __global__ void
// optimized_kernel(const double* N,
//                  const double* M,
//                  double* P,
//                  int size,
//                  const int MASK_WIDTH)
       ...
       const int MASK_WIDTH)
{
    ...
    double *M = nullptr, *N = nullptr, *P = nullptr;
    ...
    cudaMemcpy((void*)M,
               (const void*)M_h,
               sizeof(double) * MASK_WIDTH * MASK_WIDTH,
    ...
    no_opt_kernel<<<dimGrid, dimBlock>>>(
      (const double*)N, (const double*)M, (double*)P, size, MASK_WIDTH);
